...
export default class Receptor extends Component {
  ...
  componentWillMount() {
    getData('code').then(result => { this.setState({ code_parr_cmp: result }) });
  ...
  paymentpages = (p) => {
    ...
    if (p == 'cry') {
      ...
      fetch("https://yerbatsede.ml/get_taux.php", {
        method: 'POST',
        mode: 'no-cors',
        headers: {
        ...
        .then((responseJson) => {
          this.setState({ taux: responseJson[0].Taux })
          var taux = this.state.taux
          this.props.navigation.navigate('Payment', { nom_compte: nom_compte, numero_compte: numero_compte, motif_transfert: motif_transfert, montant_transfert: montant_transfert, operateur: operateur, code_parr: code_parr, taux: taux })
  ...
  OpenPaymentOption = () => {
    ...
    var numero_compte = this.state.numero_compte;
    var motif_transfert = this.state.motif_transfert;
    var code_saisi = this.state.code_parr;
    var montant_transfert = this.props.navigation.state.params.amount;
    ...
    else if (code_saisi == this.state.code_parr_cmp ) {
      this.setState({ erreur: 'Vous ne pouvez pas utiliser votre code' })
      this.setState({ showWarning: true })
    }
    else {
